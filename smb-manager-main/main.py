...
<TelaLogin>:
    ...
    MDLabel:
        text: '2023-v2.0 @raioramalho - ramalho.sit@gmail.com'
        size_hint_x: .5
    ...
    def abrir_tela_painel(self, host, usrname, password):
        ...
        try:
            ...
            if output == usrname:
                ...
                Snackbar(text="Bem-vindo: "+usrname).open()
                pass
            else:
                debug_msg('error0')
                Snackbar(text="error0").open()
            ...
            Snackbar(text="Erro de conexão com o servidor!").open()
            pass
...
class TelaPainel(MDCard):
    ...
    def verify_user(self, get_user):
        ...
        if GetUsrName == get_user:
            Snackbar(text="Usuario '"+get_user+"' encontrado!").open()
            ...
            Snackbar(text="Usuario '"+get_user+"' não encontrado!").open()
    ...
    def modify_user(self, get_user, get_full_name, get_grp):
        ...
        Snackbar(text="Modificação executada, favor verificar para confirmar!").open()
    ...
    def remove_user(self, get_user):
        ...
        Snackbar(text="Usuario '"+get_user+"' removido com Sucesso!").open()

    def create_user(sef, get_user):
        debug_msg("create_user")
        out = execute_query("""( echo '1q2w3e@ECT'; echo '1q2w3e@ECT' ) | adduser -a """ +
                            get_user+"""""", validhost, validuser, validpass)
        execute_query("""( echo '1q2w3e@ECT'; echo '1q2w3e@ECT' ) | smbpasswd -a """ +
                      get_user+"""""", validhost, validuser, validpass)
        if out == "Adding user `"+get_user+"' ...":
            Snackbar(text="Usuario: '"+get_user+"' criado com Sucesso!").open()
        else:
            Snackbar(text="Erro ao criar usuario: '"+get_user+"'").open()
    ...
    def grabText(self, inst):
        ...
        if NewPass == NewPassConfirmation:
            ...
            Snackbar(text="Senha Alterada com Sucesso!").open()
        else:
            Snackbar(text="A senha inserida não é compativel!").open()
            debug_msg('error390')
